...
class fMRIPlot(object):
    ...
    def plot(self, figure=None):
        ...
        plot_carpet(self.func_file, self.seg_data, subplot=grid[-1], tr=self.tr, img_slice=self.img_slice, tsnr = self.tsnr, stddev = self.stddev, legend = self.legend)
...
def plot_carpet(img, atlaslabels, detrend=True, nskip=0, size=(950, 800),
    ...
    if legend:
        ...
        for i, c in enumerate(coords.tolist()):
            ...
            if img_slice and tr > 1 and ctr == 0:
                x_img = image.index_img(mt.read_image_abs_path(img), tr)
                plot_epi(x_img, bg_img=None, black_bg = False, axes=ax2, display_mode='z',
                     annotate=False, cut_coords=[img_slice], cmap='jet')
                print ("I am here")
            elif tsnr and tr > 1 and ctr == 1:
                x_img = mt.read_image_abs_path(tsnr)
                plot_epi(x_img, bg_img=None, black_bg = False, axes=ax2, display_mode='z',
                     annotate=False, cut_coords=[img_slice], cmap='jet')
                print ("I am here2")
            elif stddev and tr > 1 and ctr == 2:
                x_img = mt.read_image_abs_path(stddev)
                plot_epi(x_img, bg_img=None, black_bg = False, axes=ax2, display_mode='z',
                     annotate=False, cut_coords=[img_slice], cmap='jet')
                print ("I am here3")
            else:
                plot_img(epinii, bg_img=None, axes=ax2, display_mode='z',
