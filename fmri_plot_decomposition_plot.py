...
class fMRIPlotDecompositionPlot(object):
    """
...
def plot_carpet(img, low_rank_file, sparse_file, guass_noise_file, detrend=True, nskip=0, size=(950, 800),
  ...
  # Detrend data
    ...
    wratios = [1, 100, 20]
    gs = mgs.GridSpecFromSubplotSpec(4, 2, subplot_spec=subplot,
    ...
    ax3.imshow(low_rank_set, interpolation='nearest', aspect='auto', cmap='gray',
               vmin=v[0], vmax=v[1])
    
    ax4 = plt.subplot(gs[2,1])
    ax4.imshow(sparse_set, interpolation='nearest', aspect='auto', cmap='gray',
               vmin=v[0], vmax=v[1])
    ...
    ax5.annotate(
         "Gaussian noise", xy=(0.4, 1.15), xycoords='axes fraction',
        xytext=(0, 0), textcoords='offset points',
        va='center', ha='left', color='gray', size=3,fontsize=7,
        bbox={'boxstyle': 'round', 'fc': 'w', 'ec': 'none', 'color': 'none',
              'lw': 0, 'alpha': 0.8, 'zorder':1})
    ...
    if notr:
        ax5.set_xlabel('time (frame number)', fontdict = font_label)
    else:
        ax5.set_xlabel('time (s)', fontdict = font_label)
        
    ax5.set_ylabel('voxels', fontdict = font_label)
    labels = tr * (np.array(xticks)) * t_dec
    ax5.set_xticklabels(['%.02f' % t for t in labels.tolist()], fontdict = font_label)
    ...
    for side in ["top", "right", "bottom", "left"]:
        ...
        ax5.spines[side].set_color('none')
        ax5.spines[side].set_visible(False)
